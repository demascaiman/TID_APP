...
from PyQt5.QtWidgets import QApplication, QMainWindow, QPushButton, QFileDialog, QGraphicsScene, QMessageBox 
from PyQt5 import uic
from PyQt5.QtCore import *
from PyQt5.QtGui import *

#Clase Heredada WMainWindow (Constructor de Ventanas)
COLOR_LETRAS = (55,55,255)
class Ventana(QMainWindow):
	...
	def getfile(self):
		...
		img2 = cv2.resize(img_original, (1200,800), interpolation = cv2.INTER_CUBIC)
		cv2.imwrite('img_original2.png',img2)
		self.rutaimg.setText(str("img_original2.png"))
	...
	def reestart(self):
		if self.rutaimg.text() != "":
			ruta_image = "img_original2.png"
			img_original3 = cv2.imread("img_original2.png",1)
			img_preprocesada = self.preprocesado(ruta_image,self.umbral_sli.value(),1)
			img_procesada = self.procesado(img_preprocesada, img_original3)
			cv2.imwrite('___________img_procesada.png', img_procesada)
			img_final2 = self.pintar(img_procesada)
			cv2.imwrite('img_final2.png',img_final2)
			img_final_pixmap = QPixmap("img_final2.png")
			self.imagen.setPixmap(img_final_pixmap)
			self.imagen.show();
	
	def main(self):
		self.getfile()
		img_original = cv2.imread("img_original2.png",1)
		img_preprocesada = self.preprocesado("img_original2.png",self.umbral_sli.value(),1)
	...
	def preprocesado(self, imgpath, umbral, erosionar):
		...
		cv2.imwrite("________escalagrises.png",gray_image)
		...
		cv2.imwrite("________binaria.png",procesada)
		preprocesada = cv2.dilate(procesada,kernel,iterations = self.erosion_sli.value())
		cv2.imwrite("________erosionada.png",preprocesada)
		return preprocesada
	...
	def procesado(self,img_preprocesada, img_original):
		...
		cv2.imwrite("________canny.png",canny)
		i=0
		for elemento in contornos:
			if cv2.arcLength(elemento,False)>self.perimetro_sli.value():
				cv2.drawContours (img_original, contornos, i, [0,255,0], 2)
	...
	def pintar(self, img_original):
		...
		for elemento in mod_array:			
			cv2.putText(img_original, str(count), elemento, cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.7,COLOR_LETRAS,2)
			count = count + 1
	...
	def pintar2(self, img_original):
		...
		for elemento in mod_array:	
			if elemento != (-1,-1):
				cv2.putText(img_original, str(contador), elemento, cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.7,COLOR_LETRAS,2)
				count = count + 1
			contador = contador + 1
	...
	def getPixel(self, event):
		...
		array_xy.append((x,y))
		self.add_number2()
		QMessageBox.information(self, 'Número Añadido!','Coordenadas x:'+str(x)+' y:'+str(y)) 
